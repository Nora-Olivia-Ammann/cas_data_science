...
\begin{tikzpicture}[
  ...
  bigbox/.style={draw, rounded corners, inner sep=6mm},
...
\node[box, below=of emb] (pos) {Positional\\Encoding};

% Transformer block (big box containing two inner boxes)
\node[innerbox, below=12mm of pos] (attn) {Attention};
\node[innerbox, below=12mm of attn] (ffn) {Feedforward};

\node[bigbox, fit=(attn)(ffn)] (block) {1-n Transformer Blocks};

% Continue pipeline
\node[box, below=12mm of ffn] (softmax) {Softmax};
